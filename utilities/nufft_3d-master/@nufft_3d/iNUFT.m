...
function im = iNUFT(obj,raw,maxit,damp,W,verbose,constraint,lambda)
...
% -verbose: print command line output (1=yes 0=no)
%
...
if size(raw,1)==nrow
    nc = size(raw,2);
    if verbose; fprintf('  %s received raw data: nr=%i nc=%i\n',mfilename,nrow,nc); end
else
    ...
    nc = size(raw,3);
    if verbose; fprintf('  %s received raw data: nr=%i ny=%i nc=%i\n',mfilename,nr,ny,nc); end
end
    ...
    if isequal(constraint,'parallel-imaging-pruno')
        ...
        if 0
            ...
            for j = 1:N
                ...
                d(j) = real(tmp(j)); if verbose; fprintf('%i/%i\n',j,N); end
            end
