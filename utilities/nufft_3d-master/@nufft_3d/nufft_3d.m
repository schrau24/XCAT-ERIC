classdef nufft_3d
    ...
    %  varargin = string/value pairs, e.g. ('radial',0) ('suppress', 1)
    %
    % Output:
    ...
    properties (SetAccess = private)
        ...
        verbose(1,1) logical   = 0      % print command line output (1=yes 0=no)
...
%             if obj.N(3)>1; ok = ok & kz >= -obj.K(3)/2 & kz <= obj.K(3)/2-1; end % allow 2d
            ok = kx >= -obj.K(1)/2 & kx <= obj.K(1)/2 & ky >= -obj.K(2)/2 & ky <= obj.K(2)/2;
            if obj.N(3)>1; ok = ok & kz >= -obj.K(3)/2 & kz <= obj.K(3)/2; end % allow 2d
            if obj.verbose
